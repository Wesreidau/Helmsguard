/obj/item/viking_horn
	name = "northern warhorn"
	desc = "Used to coordinate raids and strike fear into the enemy."
	icon = 'modular_hearthstone/icons/obj/items/signalhorn.dmi'
	icon_state = "northernhorn"
	slot_flags = ITEM_SLOT_HIP|ITEM_SLOT_NECK
	w_class = WEIGHT_CLASS_NORMAL

/obj/item/viking_horn/attack_self(mob/living/user)
	. = ..()
	if(istype(SSjob.GetJob(user.job), /datum/job/roguetown/highking))
		var/inputty = input("Make an announcement", "ROGUETOWN") as text|null
		if(inputty)
			if(user.gender == MALE)
				priority_announce("[inputty]", title = "The Northern King announces", sound = 'modular_hearthstone/sound/items/northking_announcement.ogg')
			else
				priority_announce("[inputty]", title = "The Northern Queen announces", sound = 'modular_hearthstone/sound/items/northking_announcement.ogg')
	else
		user.visible_message(span_warning("[user] is about to sound the [src]!"))
		if(do_after(user, 15))
			sound_horn(user)

/obj/item/viking_horn/proc/sound_horn(mob/living/user)
	...
	switch(user.job)
		...
		if("Northern Serf", "Northmen Berserkir", "Northmen Ulfhednar")
			playsound(src, 'modular_hearthstone/sound/items/northhorn.ogg', 100, TRUE)
		else
	...
	for(var/mob/living/player in GLOB.player_list)
		...
		switch(user.job)
			...
			if("Northern Serf", "Northmen Berserkir", "Northmen Ulfhednar")
				player.playsound_local(get_turf(player), 'modular_hearthstone/sound/items/northhorn.ogg', 35, FALSE, pressure_affected = FALSE)
				to_chat(player, span_warning("I hear the horn of the Northmen somewhere[disttext][dirtext]!"))
			else
